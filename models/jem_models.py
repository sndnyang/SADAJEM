...
class F(nn.Module):
    def __init__(self, depth=28, width=2, norm=None, dropout_rate=0.0, n_classes=10, model='wrn', args=None):
        ...
        # default, wrn
        self.norm = norm
        dataset = 'cifar10' if args is None else args.dataset

        self.f = wideresnet.Wide_ResNet(depth, width, norm=norm, dropout_rate=dropout_rate, dataset=dataset, num_classes=n_classes)
...
def get_model_and_buffer(args, device):
    ...
    if ',' in args.gpu_id:
        f = nn.DataParallel(f, device_ids=range(len(args.gpu_id.split(','))))
